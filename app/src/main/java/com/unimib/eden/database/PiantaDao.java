...
public interface PiantaDao {
     ...
     */
    @Query("SELECT * FROM 'pianta' WHERE nome LIKE '%' || :query || '%' AND esposizione_sole = :esposizioneSole AND inizio_semina >= :inizioSemina AND fine_semina <= :fineSemina")
    List<Pianta> searchPianteFiltriAll(String query, String esposizioneSole, int inizioSemina, int fineSemina);
     ...
     * @return  La lista di tutte le piante nel cui nome è presente come sottostringa la stringa passata in input e soddisfa tutti i parametri passati in input.
     */
    @Query("SELECT * FROM 'pianta' WHERE nome LIKE '%' || :query || '%' AND inizio_semina >= :inizioSemina AND fine_semina <= :fineSemina")
    List<Pianta> searchPianteFiltri(String query, int inizioSemina, int fineSemina);
